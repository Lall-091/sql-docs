...
- For each event that is successfully replayed on the target server, there's one corresponding output event class.
...
|Attention|**bigint**|The attention duration (in microseconds) for the event. This is calculated from the Attention event from the capture trace. If there was no query time-out specified for the event, this column isn't populated (null).|5|  
