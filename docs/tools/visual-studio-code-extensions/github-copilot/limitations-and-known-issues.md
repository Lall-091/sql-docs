...
- GitHub Copilot **doesn't have permission to write data**. Developers must manually review and execute all generated SQL or object-relational mapping (ORM) code.
...
- This experience is **designed for developers**, not for database or system administrators. While it can generate SQL scripts for administrative tasks, GitHub Copilot **doesn't support server management operations** such as configuring backup/restore, managing user permissions, or handling SQL Agent jobs.

- GitHub Copilot sessions **don't persist history when switching context** (for example, changing files or databases). New context resets the chat memory.
...
- The integration is **optimized for modern SQL database in Fabric, Azure SQL Database, and [!INCLUDE [sssql17-md](../../../includes/sssql17-md.md)] and later versions**. Legacy or deprecated features might be unsupported, and the language service doesn't officially support [!INCLUDE [sssql16-md](../../../includes/sssql16-md.md)] and earlier versions.

- **Azure Synapse Analytics** and its Dedicated SQL pool (formerly Azure SQL DW) features **aren't supported** by this GitHub Copilot integration. No timeline is published for adding Synapse support.

- **Inline completions don't see your database schema.** Visual Studio Code's inline completion application programming interface (API) is effectively single-provider: when GitHub Copilot is enabled, third-party extensions (including the MSSQL extension) can't contribute schema-aware ghost text. For schema-aware SQL suggestions, use the [`@mssql` chat participant](chat-ask-mode.md) instead. For the architectural reason, see [How GitHub Copilot works with the MSSQL extension](how-it-works.md).
...
- GitHub Copilot **doesn't support offline usage** or disconnected development.
...
- GitHub Copilot **doesn't have real-time knowledge of remote files** unless you open them in your editor.
- GitHub Copilot **doesn't track variable types or state across long conversations**; results might drift in relevance.

For more information, see:

- [Best practices for using GitHub Copilot](https://docs.github.com/copilot/get-started/best-practices)
- [Tips and tricks for Copilot in Visual Studio Code](https://code.visualstudio.com/docs/copilot/best-practices)
...
- [How GitHub Copilot handles data - Learning pathway](https://learn.github.com/learning-pathways/github-copilot)
