...
#### IRowsetFastLoad Rowsets  
 The OLE DB Driver for SQL Server bulk copy rowsets are write-only, but they expose interfaces that allow the consumer to determine the structure of a [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] table. The following interfaces are exposed on a bulk copy-enabled OLE DB Driver for SQL Server rowset:  
