...
SQL Server container images can use the Microsoft Distributed Transaction Coordinator (MSDTC), which is required for distributed transactions. To understand the communications requirements for MSDTC, see [How to configure the Microsoft Distributed Transaction Coordinator (MSDTC) on Linux](../configure/distributed-transactions.md).
...
For more information about routing ports, see [Configure port routing](../configure/distributed-transactions.md#configure-port-routing).
...
:::image type="content" source="media/configure-distributed-transactions/msdtc-single.png" alt-text="Diagram showing the process when an MSDTC client connects to MSDTC on SQL Server running inside a Linux container." lightbox="media/configure-distributed-transactions/msdtc-single.svg":::
