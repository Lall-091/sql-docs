...
-   Online restore requires all the backup devices of the backup. An attempt to restore with fewer devices fails.  
 ...
 For example, consider a case in which a database was backed up to four tape drives connected to the server. An online restore requires that you have four drives connected to the server; an offline restore allows you to restore the backup if there are fewer than four drives on the machine.  
...
### WITH Options  
 Specifies the options to be used by a restore operation. For a summary of which statements use each option, see "Summary of Support for WITH Options," later in this article.  
...
**Applies to**: [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] SP1 CU2 and later
 ...
 Used only when restoring a backup from the Microsoft Azure Blob Storage.  
...
## Result sets  
 For result sets, see the following articles:  
...
## Remarks  
 For additional remarks, see the following articles:  
...
## Permissions  
 For permissions, see the following articles:  
