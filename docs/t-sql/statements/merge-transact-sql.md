...
description: The MERGE statement runs insert, update, or delete operations on a target table from the results of a join with a source table.
...
::: moniker range="=azure-sqldw-latest || =fabric"
...
Specifies the number or percentage of affected rows. *expression* can be either a number or a percentage of the rows. The rows referenced in the `TOP` expression aren't arranged in any order. For more information, see [TOP (Transact-SQL)](../queries/top-transact-sql.md).
...
For more information about the syntax and arguments of this clause, see [FROM (Transact-SQL)](../queries/from-transact-sql.md).
...
`NOLOCK` and `READUNCOMMITTED` aren't allowed. For more information about table hints, see [Table hints (Transact-SQL)](../queries/hints-transact-sql-table.md).
...
Specifies the name or ID of one or more indexes on the target table for doing an implicit join with the source table. For more information, see [Table Hints (Transact-SQL)](../queries/hints-transact-sql-table.md).

#### \<output_clause>

Returns a row for every row in *target_table* that's updated, inserted, or deleted, in no particular order. `$action` can be specified in the output clause. `$action` is a column of type **nvarchar(10)** that returns one of three values for each row: `INSERT`, `UPDATE`, or `DELETE`, according to the action done on that row. The `OUTPUT` clause is the recommended way to query or count rows affected by a `MERGE`. For more information about the arguments and behavior of this clause, see [OUTPUT clause (Transact-SQL)](../queries/output-clause-transact-sql.md).

::: moniker-end

#### OPTION ( \<query_hint> [ ,...n ] )

Specifies that optimizer hints are used to customize the way the Database Engine processes the statement. For more information, see [Query hints (Transact-SQL)](../queries/hints-transact-sql-query.md).
...
For more information about the arguments of this clause, see [UPDATE (Transact-SQL)](../queries/update-transact-sql.md). Setting a variable to the same value as a column isn't supported.
...
For more information about this clause, see [INSERT (Transact-SQL)](insert-transact-sql.md).
::: moniker-end
...
When used after `MERGE`, [@@ROWCOUNT (Transact-SQL)](../functions/rowcount-transact-sql.md) returns the total number of rows inserted, updated, and deleted to the client.
...
> [!CAUTION]
> Don't use the `MERGE` statement when using [queued updating replication](../../relational-databases/replication/transactional/updatable-subscriptions-queued-updating-conflict-resolution.md). The `MERGE` and queued updating trigger aren't compatible. Replace the `MERGE` statement with an `INSERT` and `UPDATE` statements.
...
Requires `SELECT` permission on the source table and `INSERT`, `UPDATE`, or `DELETE` permissions on the target table. For more information, see the Permissions section in the [SELECT (Transact-SQL)](../queries/select-transact-sql.md), [INSERT (Transact-SQL)](insert-transact-sql.md), [UPDATE (Transact-SQL)](../queries/update-transact-sql.md), and [DELETE (Transact-SQL)](delete-transact-sql.md) articles.

<a id="optimizing-merge-statement-performance"></a>
...
<a id="b-using-merge-to-do-update-and-delete-operations-on-a-table-in-a-single-statement"></a>
...
The `OUTPUT` clause can be useful to query the result of `MERGE` statements, for more information, see [OUTPUT clause (Transact-SQL)](../queries/output-clause-transact-sql.md). The example also shows how to store the results of the `OUTPUT` clause in a table variable. And, then you summarize the results of the `MERGE` statement by running a simple select operation that returns the count of inserted and updated rows.
...
- [Table Value Constructor (Transact-SQL)](../queries/table-value-constructor-transact-sql.md)
