...
You can only specify the key-value pairs that have an entry in [DSN and Connection String Keywords and Attributes](../../connect/odbc/dsn-connection-string-attribute.md) under the **DSN / Connection String Keyword** column. For example, the `SQL_ATTR_TXN_ISOLATION` keyword isn't supported, because that is an attribute set using **SQLSetConnectAttr**, not in the connection string.
...
| `PolyBaseQoTopPushdownSyntax` | `TOP`, `LIMIT` | Contains information specifying how to push down the `TOP` operator to the backend. The default is an empty string, indicating a lack of support for `TOP` pushdown. If the user specifies `TOP`, `top {0}` is used as the format string. If the user specifies `LIMIT`, `limit {0}` is used as the format string. This implementation is driver-specific, consult the external data source and/or driver documentation. For example: `CONNECTION_OPTIONS=PolyBaseQoTopPushdownSyntax=TOP'`. |
