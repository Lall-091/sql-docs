...
1. Declare a variable of type **nvarchar(3000)** to hold the error description.

1. Declare a variable of type **xml** to hold an XML representation of the message body.

1. `CAST` the message body from **varbinary(max)** to **xml**, and assign the results to the variable of type **xml**.

1. Use the value function of the **xml** data type to retrieve the error code.

1. Use the value function of the **xml** data type to retrieve the error description.

1. Handle the error as appropriate for your application. Errors with negative error codes are generated by Service Broker. Errors with positive error codes are generated by service programs that ran `END CONVERSATION WITH ERROR`.
