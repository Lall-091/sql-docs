...
Service Broker endpoints set options for the transport protocol. SQL Server doesn't contain Service Broker endpoints by default. For more information on creating a Service Broker endpoint, see [How to: Activate Service Broker networking](how-to-activate-service-broker-networking-transact-sql.md).
...
A sender retains message fragments that the receiver hasn't acknowledged. If no acknowledgment is received within a system-defined wait time, the sender sends the message fragment again. If no acknowledgment is received during the wait time, Service Broker exponentially increases the amount of time before the next retry, up to a maximum wait time. The initial wait time for a retry is a few seconds. The maximum wait time is approximately one minute. The wait time isn't intended to be precise; depending on network traffic and the other activity in the SQL Server instance, a message fragment might not be retried for a few seconds after the wait time expires.

If an acknowledgment is lost or delayed, the recipient might receive duplicate messages. In this case, the recipient acknowledges receipt of the duplicate message, but doesn't deliver the duplicate message to the queue.

Service Broker uses message acknowledgment to provide reliable messaging without distributed transactions. A recipient sends an acknowledgment only after adding the message or message fragment to the queue. The sender holds the message in the transmission queue until the acknowledgment for that message arrives. Although the sender and the recipient never share a transaction, the protocol guarantees that the sender doesn't remove the message from the transmission queue until the recipient has successfully received the message.
...
The destination for the message decrypts the message, and then compares the signature in the message to a new signature computed over the actual contents received. If the signatures don't match, the message has been damaged or tampered with during transmission. The message fails the message integrity check. SQL Server discards the message and doesn't acknowledge the message to the sender. The **Broker:Corrupted Message** event class reports information when a message fails the message integrity check.
...
- Sends a message through the transmission queue. This includes the following:
  ...
  - Messages to sent queues in the local instance if the message can't be directly inserted into the destination queue
...
To free memory, Service Broker periodically stores batches of inactive transmission objects in `tempdb` work tables. When a transmission object is first modified in memory it's marked as dirty. The transmission object remains marked as dirty until it's flushed to a work table.

Transmission objects aren't used for either sending or receiving local messages that can be inserted directly into the destination queue.
...
:::image type="content" source="media/broker-network-communication.png" alt-text="Diagram of Broker network communication between two instances.":::

The conversation is a persistent, logical connection. The conversation can occur over any period of time, and during that period of time, the conversation can use any number of network connections.
...
To deliver a message, Service Broker holds the message in the transmission queue for the database that sent the message. The recipient delivers the message directly to the queue for the destination service. If that queue is `OFF`, the message is held temporarily in the transmission queue for the receiving database. The queue for the sending service isn't involved in the operation. The transmission queue for the database that hosts the receiving service is only involved if the destination queue is `OFF`.
