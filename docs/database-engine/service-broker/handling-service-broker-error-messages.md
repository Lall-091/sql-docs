...
## Report application error conditions
...
You can use application-defined error messages to report errors that aren't severe enough to end a conversation. The application designer can specify the following:
...
Error handling routines should be coded in such a way as to prevent poison messages. For more information, see [Handle poison messages](handling-poison-messages.md).
