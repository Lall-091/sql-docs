...
- For a database principal to send messages to a service, that principal must have `SEND` permissions on the service. For services hosted in a different instance, Service Broker dialog security determines the database principal in the remote instance. For more information, see [Service Broker dialog security](service-broker-dialog-security.md). Service Broker doesn't consider membership in Windows roles when checking `SEND` permissions.
