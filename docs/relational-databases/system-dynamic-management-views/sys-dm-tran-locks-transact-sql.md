...
On [!INCLUDE [ssNoVersion_md](../../includes/ssnoversion-md.md)] and SQL Managed Instance, `sys.dm_tran_locks` requires `VIEW SERVER STATE` permission.
...
In [!INCLUDE [fabric-se-dw](../../includes/fabric-se-dw.md)], membership in the Viewer workspace role is the minimum permission necessary to view `sys.dm_tran_locks`.
