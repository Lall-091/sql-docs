...
| 4106 | 15 | No | The function '%.\*ls' may not have a window frame. |
...
| 4123 | 15 | No | Window element in OVER clause can not also be specified in WINDOW clause. |
| 4124 | 16 | No | The parameters supplied for the batch are not valid. |
| 4125 | 15 | No | The function '%.\*ls' must have an OVER clause. |
...
| 4222 | 16 | No | The log backup cannot be performed with the provided MAX_VLF_COUNT option. |
| 4223 | 16 | No | The log backup cannot be performed with the provided MAX_VLF_COUNT option on Hadron secondaries. |
...
| 4713 | 16 | No | Truncate table failed for '%.\*ls' with error number %d |
| 4714 | 16 | No | Truncating global temporary tables and transient user tables in TEMPDB is not supported. |
| 4715 | 16 | No | Truncating an external table is not allowed. |
...
| 4879 | 16 | No | Bulk load failed due to invalid column value in CSV data file %ls in row %ls%I64d, column %d. |
| 4880 | 16 | No | Cannot bulk load. When you use the FIRSTROW/FIRST_ROW and LASTROW parameters, the value for FIRSTROW/FIRST_ROW cannot be greater than the value for LASTROW. |
