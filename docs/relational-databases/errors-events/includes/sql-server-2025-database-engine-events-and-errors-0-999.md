...
| 369 | 16 | No | This operation is not supported on Azure SQL Database with the TRCFLG_AUTO_PARTITION_REPLICATE traceflag turned on. |
| 370 | 16 | No | Unknown object name. |
...
| 810 | 16 | Yes | The operating system returned error %ls to SQL Server during a %S_MSG at offset %#016I64x in file '%ls'. This is intended to raise only in Azure SQL DB. Complete a full database consistency check (DBCC CHECKDB). This error can be caused by many factors; for more information, see SQL Server Books Online. |
...
| 837 | 17 | Yes | The operating system returned error %ls to SQL Server during a %S_MSG at offset %#016I64x in file '%ls'. This error can be caused when a remote IO fails due to timeout. |
| 838 | 10 | No | An asynchronous API call (%ls) hasn't returned in %I64u ms. Non-yielding scheduler errors might occur. |
...
| [846](../mssqlserver-846-database-engine-error.md) | 10 | No | A time-out occurred while waiting for buffer latch -- type %d, bp %p, page %d:%d, stat %#x, database id: %d, allocation unit Id: %I64d%ls, task 0x%p : %d, waittime %d seconds, flags 0x%I64x, owning task 0x%p. Not continuing to wait. %ls |
...
| 900 | 16 | No | Failed to start buffer pool extension because the file configuration has changed and is incompatible with the existing one. |
