...
Or you can use `CONVERT`, as shown in the following code sample:
...
You can also use the [TYPE directive in FOR XML queries](type-directive-in-for-xml-queries.md) in the `FOR XML` clause that directly returns a `FOR XML` query result as **xml** type:
