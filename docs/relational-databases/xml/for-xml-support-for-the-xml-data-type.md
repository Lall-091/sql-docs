...
If you specify the `ELEMENTS` directive in the query, the `BusinessEntityID`, `LastName`, and `FirstName` are returned as elements in the resulting XML.
...
`FOR XML` queries can be used for returning XML from a user-defined function that returns either of:
