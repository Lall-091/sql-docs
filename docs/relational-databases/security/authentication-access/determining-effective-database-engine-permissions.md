...
> [!NOTE]  
> The information in this article applies to [!INCLUDE [ssversion2005-md](../../../includes/ssversion2005-md.md)] and later versions. Some types of permissions aren't available in some versions of [!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)].
...
- In Fabric SQL database, Microsoft Entra ID for database users is the only supported authentication method. Server-level roles and permissions aren't available, only database-level. For more information, see [Authorization in SQL database in Microsoft Fabric](/fabric/database/sql/authorization).
...
> [!NOTE]  
> Does not apply to SQL Database or Azure Synapse Analytics where server level permission isn't available. The `is_fixed_role` column of `sys.server_principals` was added in [!INCLUDE [sssql11-md](../../../includes/sssql11-md.md)]. It isn't needed for older versions of [!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)].
...
> [!NOTE]  
> All logins are members of the public role and can't be removed. The query checks tables in the `master` database, but it can be executed in any database for the on-premises product.
...
To understand the permissions that are granted to each role, see the role descriptions in [Server-level roles](../../../relational-databases/security/authentication-access/server-level-roles.md), and [Database-level roles](../../../relational-databases/security/authentication-access/database-level-roles.md).
...
The following query returns a list of the permissions that have been granted or denied at the database level. Run this query in each database.
...
Each class of permission in the permission table can be joined to other system views that provide related information about that class of securable. For example, the following query provides the name of the database object that is affected by the permission.
