...
Real-time analytics uses an updateable nonclustered columnstore index on a rowstore table. The columnstore index maintains a copy of the data, so the OLTP and analytics workloads run against separate copies of the data. This minimizes the performance impact of both workloads running at the same time. The [!INCLUDE [ssDE-md](../../includes/ssde-md.md)] automatically maintains index changes so OLTP changes are always up-to-date for analytics. With this design, it is possible and practical to run analytics in real time on up-to-date data. This works for both disk-based and memory-optimized tables.
...
1. For each table, drop all B-tree indexes that are primarily designed to speed up existing analytics on your OLTP workload. Replace them with a single nonclustered columnstore index. This can improve the overall performance of your OLTP workload since there are fewer indexes to maintain.
    ...
    ```
     ...
     The columnstore index on a memory-optimized table allows operational analytics by integrating In-memory OLTP and columnstore technologies to deliver high performance for both OLTP and analytics workloads. The columnstore index on a memory-optimized table must be the clustered index, in other words it must include all columns.
...
You are now ready to run real-time operational analytics without making any changes to your application. Analytics queries will run against the columnstore index and OLTP operations will keep running against your OLTP B-tree indexes. The OLTP workloads continue to perform, but incur some additional overhead to maintain the columnstore index. See the performance optimizations in the next section.
...
> [!NOTE]  
> The Query Optimizer considers, but doesn't always choose, the columnstore index for the query plan. When the query optimizer chooses the filtered columnstore index, it transparently combines the rows both from columnstore index as well as the rows that do not meet the filtered condition to allow real-time analytics. This is different from a regular nonclustered filtered index which can be used only in queries that restrict themselves to the rows present in the index.
...
The analytics query executes with the following query plan. You can see that the rows not meeting the filtered condition are accessed through clustered B-tree index.
...
The default value of `COMPRESSION_DELAY` option should work for most customers.
