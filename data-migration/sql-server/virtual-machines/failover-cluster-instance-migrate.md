...
This article teaches you to migrate your Always On failover cluster instance (FCI) to SQL Server on Azure VMs using the [Azure Migrate: Server Migration tool](/azure/migrate/migrate-services-overview#azure-migrate-server-migration-tool). Using the migration tool, you can migrate each node in the failover cluster instance to an Azure VM hosting SQL Server, as well as the cluster and FCI metadata.
...
1. In **Download and install the replication appliance software**, download the appliance installer, and the registration key. You need to the key in order to register the appliance. The key is valid for five days after it's downloaded.
   ...
   :::image type="content" source="media/availability-group-migrate/download-provider.png" alt-text="Screenshot of the download provider option." lightbox="media/availability-group-migrate/download-provider.png":::
...
:::image type="content" source="media/availability-group-migrate/discovered-servers.png" alt-text="Screenshot of the Discovered servers option.":::
...
1. In **Target settings**, select the subscription, and target region to which you're migrating, and specify the resource group in which the Azure VMs will reside after migration.
...
1. In **Disk encryption type**, select:
   ...
   > [!NOTE]  
   > To replicate VMs with CMK, you need to [create a disk encryption set](/azure/virtual-machines/disks-enable-customer-managed-keys-portal#set-up-your-disk-encryption-set) under the target Resource Group. A disk encryption set object maps Managed Disks to a Key Vault that contains the CMK to use for SSE.
...
1. In **Azure Hybrid Benefit**:
   ...
   :::image type="content" source="media/availability-group-migrate/target-settings.png" alt-text="Screenshot of the Target settings option." lightbox="media/availability-group-migrate/target-settings.png":::
...
1. In **Disks**, specify whether the VM disks should be replicated to Azure, and select the disk type (Standard SSD/HDD or premium managed disks) in Azure. Then select **Next**.
   ...
   :::image type="content" source="media/availability-group-migrate/disks.png" alt-text="Screenshot of the Disk settings option." lightbox="media/availability-group-migrate/disks.png":::
...
:::image type="content" source="media/availability-group-migrate/replicating-servers.png" alt-text="Screenshot of the Monitor replication option.":::
...
- For SQL Server:
  - Install [SQL Server IaaS Agent extension](/azure/azure-sql/virtual-machines/windows/sql-server-iaas-agent-extension-automate-management) to automate management and administration tasks. The SQL IaaS Agent extension only supports limited functionality on SQL Server failover clustered instances.
...
- [Get started: Accelerate migration](/azure/cloud-adoption-framework/get-started/migrate)
