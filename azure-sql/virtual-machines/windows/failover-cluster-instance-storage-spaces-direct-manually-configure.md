...
[Storage Spaces Direct (S2D)](/windows-server/storage/storage-spaces/storage-spaces-direct-overview) supports two types of architectures: converged and hyperconverged. A hyperconverged infrastructure places the storage on the same servers that host the clustered application, so each SQL Server FCI node has its own storage.
...
- An Azure load balancer holds the IP address for the SQL Server FCI for a single subnet scenario.
...
To validate the cluster by using the UI, complete the following steps on one of the virtual machines:
...
1. On the  **Test Selection** page, select all tests *except* **Storage**:
   ...
   ![Screenshot of the Select cluster validation tests.](./media/failover-cluster-instance-storage-spaces-direct-manually-configure/10-validate-cluster-test.png)
...
1. [Create a volume](/windows-server/storage/storage-spaces/deploy-storage-spaces-direct#step-36-create-volumes).
   ...
   After you run the preceding command, an 800-GB volume is mounted as a cluster resource. The volume is at `C:\ClusterStorage\Volume1\`.
...
:::image type="content" source="media/failover-cluster-instance-premium-file-share-manually-configure/test-cluster-failover.png" alt-text="Screenshot that shows Test cluster failover by moving the core resource to the other nodes.":::
...
1. If the version of the operating system is Windows Server 2019 and the Windows Cluster was created using the default [**Distributed Network Name (DNN)**](https://blogs.windows.com/windows-insider/2018/08/14/announcing-windows-server-2019-insider-preview-build-17733/), the FCI installation for SQL Server 2017 and earlier versions fails with the error `The given key was not present in the dictionary`.

   During installation, SQL Server setup queries for the existing Virtual Network Name (VNN) and doesn't recognize the Windows Cluster DNN. The issue is fixed in SQL Server 2019 and later versions. For SQL Server 2017 and earlier versions, follow these steps to avoid the installation error:
...
1. In **Database Engine Configuration**, the FCI data directories need to be on clustered storage. With Storage Spaces Direct, it's not a shared disk but a mount point to a volume on each server. Storage Spaces Direct synchronizes the volume between both nodes. The volume is presented to the cluster as a CSV. Use the CSV mount point for the data directories.
   ...
   ![Screenshot of the Data directories.](./media/failover-cluster-instance-storage-spaces-direct-manually-configure/20-data-dicrectories.png)
...
1. After the FCI installation succeeds on the first node, connect to the second node by using [Bastion](/azure/bastion/bastion-connect-vm-rdp-windows).
...
1. For a multi-subnet scenario, in **Cluster Network Configuration**, enter the secondary IP address in the subnet of the _second_ SQL Server VM that you previously designated as the [IP address of the failover cluster instance network name](failover-cluster-instance-prepare-vm.md#assign-secondary-ip-addresses).
   ...
   :::image type="content" source="./media/failover-cluster-instance-azure-shared-disk-manually-configure/sql-install-cluster-network-secondary-ip-vm-2.png" alt-text="Screenshot that enters the secondary IP address in the subnet of the second SQL Server VM subnet.":::
...
To manage your SQL Server VM from the portal, register it with the [SQL IaaS Agent extension](sql-agent-extension-manually-register-single-vm.md). SQL Server VMs with failover cluster instances have only [limited functionality](sql-server-iaas-agent-extension-automate-management.md#feature-benefits).

> [!NOTE]  
> Currently, SQL Server failover cluster instances on Azure virtual machines registered with the SQL IaaS Agent extension only support a [limited](failover-cluster-instance-overview.md#limited-extension-support) number of features available through basic registration. They don't support features that require the agent, such as automated backup, patching, Microsoft Entra authentication, and advanced portal management. To learn more, see the [table of benefits](sql-server-iaas-agent-extension-automate-management.md#feature-benefits).

Register a SQL Server VM by using PowerShell. The `-LicenseType` parameter can be `PAYG` or `AHUB`:
...
- You can use Storage Spaces Direct only if you uncheck or clear the disk eligibility option when adding storage to the cluster for disks that you attach as NTFS-formatted disks.
- SQL Server FCIs registered with the SQL IaaS Agent extension don't support features that require the agent, such as automated backup, patching, Microsoft Entra authentication, and advanced portal management. For more information, see the [table of benefits](sql-server-iaas-agent-extension-automate-management.md#feature-benefits).
- Failover cluster instances that use Storage Spaces Direct as the shared storage don't support using a disk witness for the quorum of the cluster. Use a cloud witness instead.

- Microsoft Distributed Transaction Coordinator (MSDTC) isn't supported by SQL Server on Azure VM failover cluster instances deployed to Storage Spaces direct. Review [FCI limitations](failover-cluster-instance-overview.md#limitations) for more information.
- Microsoft Distributed Transaction Coordinator (MSDTC) is supported on Azure virtual machines starting with Windows Server 2019 and later when deployed to dedicated Clustered Shared Volumes (CSVs) and using a [standard load balancer](/azure/load-balancer/load-balancer-overview). MSDTC isn't supported on Windows Server 2016 and earlier.

[!INCLUDE [virtual-machines-fci-limitations](../../includes/virtual-machines-fci-limitations.md)]

## Next steps

If Storage Spaces Direct isn't the right FCI storage solution for you, consider creating your FCI by using [Azure shared disks](failover-cluster-instance-azure-shared-disks-manually-configure.md) or [Premium File Shares](failover-cluster-instance-premium-file-share-manually-configure.md) instead. 
