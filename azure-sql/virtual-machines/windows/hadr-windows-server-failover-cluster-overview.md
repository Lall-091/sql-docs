...
This article describes the differences when using the Windows Server Failover Cluster feature with SQL Server on Azure virtual machines (VMs) for high availability and disaster recovery (HADR). Such as, for Always On availability groups (AG) or failover cluster instances (FCI).
...
In order to provide high availability, the cluster must ensure the health of the different components that make up the clustered solution. The cluster service monitors several system and network parameters to evaluate the health of the cluster in order to detect and respond to failures.
...
|-|-|
| Aggressive | Provides rapid failure detection and recovery of hard failures, which delivers the highest levels of availability. The cluster service and SQL Server are both less forgiving of transient failure and in some situations might prematurely fail over resources when there are transient outages. Once failure is detected, the corrective action that follows can take extra time. |
...
Configuration of the VNN can be cumbersome, it's an added source of failure, it can cause a delay in failure detection, and there's an overhead and cost associated with managing another resource. To address some of these limitations, SQL Server introduced support for the Distributed Network Name feature.
...
- Failover duration is minimized when the load balancer probes are eliminated.
...
Most SQL Server features work transparently with FCI and availability groups when using the DNN, but there are certain features that might require special consideration.
...
There are more considerations when using the DNN with other SQL Server features. To learn more, see [FCI and DNN interoperability](failover-cluster-instance-dnn-interoperability.md) and [AG and DNN interoperability](availability-group-dnn-interoperability.md).
...
The cluster service takes corrective action when a failure is detected. This action could restart the resource on the existing node, or fail the resource over to another node. Once corrective measures are initiated, they make take some time to complete.
...
1. Individual databases go through recovery, which can take some time depending on several factors, such as the length of the redo log. The listener doesn't route connections until the database is fully recovered. To learn more, see [Estimating failover time (RTO)](/sql/database-engine/availability-groups/windows/monitor-performance-for-always-on-availability-groups).
...
Nodes in a Windows cluster on virtual machines in Azure might be physically separated within the same Azure region, or they can be in different regions. The distance can introduce network latency much like having cluster nodes spread between locations in your own facilities would. In cloud environments, the difference is that within a region you might not be aware of the distance between nodes. Moreover, some other factors like physical and virtual components, number of hops, etc. can also contribute to increased latency. If latency between the nodes is a concern, consider placing the nodes of the cluster within a [proximity placement group](/azure/virtual-machines/co-location) to guarantee network proximity.

## Resource limits

When you configure an Azure VM, you determine the computing resources limits for the CPU, memory, and IO. Workloads that require more resources than the purchased Azure VM, or disk limits can cause VM performance issues. Performance degradation might result in a failed health check for either the cluster service, or for the SQL Server high availability feature. Resource bottlenecks might make the node or resource appear down to the cluster or SQL Server.
...
If your SQL Server is experiencing unexpected failovers, check to make sure you're following all [performance best practices](performance-guidelines-best-practices-checklist.md) and monitor the server for disk or VM-level capping.
...
A resource bottleneck during platform  maintenance might make the AG or FCI appear down to the cluster service. To learn more, see the [resource limits](#resource-limits) section of this article.

If you're using aggressive cluster monitoring, an extended VM pause might trigger a failover. A failover often causes more downtime than the maintenance pause. So, using relaxed monitoring to avoid triggering a failover while the VM is paused for maintenance is recommended. For more information on setting cluster thresholds in Azure VMs, see the [Cluster best practices](hadr-cluster-best-practices.md).
