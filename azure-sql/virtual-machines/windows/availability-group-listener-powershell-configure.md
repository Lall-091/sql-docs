...
Add only the primary IP address of the VM to the back-end pool of the load balancer as the [secondary VM IP address doesn't support floating IP](/azure/load-balancer/load-balancer-floating-ip).
...
1. Use [Bastion](/azure/bastion/bastion-connect-vm-rdp-windows) to connect to a SQL Server that is in the same virtual network, but doesn't own the replica. It might be the other SQL Server in the cluster.
