...
After you create the network, subnet, and availability sets, you're ready to create and configure domain controllers.

### Create virtual machines for the domain controllers

Now, create two virtual machines. Name them **ad-primary-dc** and **ad-secondary-dc**. Use the following steps for each virtual machine:

1. Return to the **SQL-HA-RG** resource group.
1. Select **Add**.
1. Type **Windows Server 2016 Datacenter**, and then select **Windows Server 2016 Datacenter**.
...
> [!IMPORTANT]  
> You can place a virtual machine (VM) in an availability set only when you create it. You can't change the availability set after a VM is created. See [Manage the availability of virtual machines](/azure/virtual-machines/availability).
...
1. Sign in with your configured administrator account (**\DomainAdmin**).
...
1. In the **Active Directory Domain Services Configuration Wizard**, use the following values:
   ...
   | **Domain Controller Options** |**DSRM Password** = secure password<br /><br />**Confirm Password** = secure password |
...
The preferred DNS server address [shouldn't be updated](/azure/virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances#specify-dns-servers) directly within a VM, it should be edited from the [Azure portal, PowerShell, or Azure CLI](/azure/virtual-network/virtual-network-network-interface?tabs=network-interface-portal#change-dns-servers). The following steps show how to make the change inside of the Azure portal:
...
1. Select either:
   ...
   - **Inherit from virtual network**: Choose this option to inherit the DNS server setting defined for the virtual network that the network interface is assigned to. Selecting this option would automatically inherit the primary domain controller as the DNS server.
...
1. Select **Save**. If using a **Custom** DNS Server, return to the virtual machine in the Azure portal and restart the VM. Once the virtual machine is restarted, you can join the VM to the domain.
...
|---------|---------|---------|---------|
|Install    |Both| Corp\Install        |Sign in to either VM with this account to configure the cluster and availability group. |
...
1. On the **Tasks** pane, select **New**, and then select **User**.
   ...
   > [!TIP]  
   > Set a complex password for each account. For nonproduction environments, set the user account to never expire.
...
Now that you're finished configuring Active Directory and the user objects, you can create more VMs and join them to the domain.
...
Windows Server 2016 and later versions can use a [cloud witness](/windows-server/failover-clustering/deploy-cloud-witness). But for consistency with previous operating systems, this article uses a virtual machine for a witness.
...
- **Network: Number of NICs per server**
  ...
  Use a single network interface card (NIC) per server (cluster node) and a single subnet. Azure networking has physical redundancy, which makes adding more NICs and subnets unnecessary on an Azure VM guest cluster.

  The cluster validation report warns you that the nodes are reachable only on a single network. You can ignore this warning on Azure VM guest failover clusters.
...
1. To finish creating the three VMs, use the information in the following table:
   ...
   | Virtual machine configuration: **Basics** |**Name** = **cluster-fsw**<br /><br />**User Name** = **DomainAdmin**<br /><br />**Password** = secure password<br /><br />**Subscription** = Your subscription<br /><br />**Resource group** = **SQL-HA-RG**<br /><br />**Location** = Your Azure location |**Name** = **sqlserver-0**<br /><br />**User Name** = **DomainAdmin**<br /><br />**Password** = secure password<br /><br />**Subscription** = Your subscription<br /><br />**Resource group** = **SQL-HA-RG**<br /><br />**Location** = Your Azure location |**Name** = **sqlserver-1**<br /><br />**User Name** = **DomainAdmin**<br /><br />**Password** = secure password<br /><br />**Subscription** = Your subscription<br /><br />**Resource group** = **SQL-HA-RG**<br /><br />**Location** = Your Azure location |
...
> [!NOTE]  
> You can now automate this task, along with actually joining the SQL Server VMs to the failover cluster. Use the [Azure CLI](availability-group-az-commandline-configure.md) and [Azure Quickstart Templates](availability-group-quickstart-template-configure.md).
...
- [Tutorial: Manually configure an availability group - SQL Server on Azure VMs](availability-group-manually-configure-tutorial-single-subnet.md)
