...
:::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-multi-subnet/multi-subnet-availability-group-diagram.png" alt-text="Diagram illustrating the resources you deploy in this tutorial":::
...
1. Select **+ Create a resource** to create a new resource in the portal.
   ...
   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-multi-subnet/01-portal-plus.png" alt-text="Screenshot with +Create a resource selected in the Azure portal.":::
...
1. Search for **resource group** in the **Marketplace** search box, and choose the **Resource group** tile from Microsoft. Select **Create** on the **Resource group** page.
   ...
   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-multi-subnet/01-resource-group-search.png" alt-text="Screenshot of the resource group tile in Azure Marketplace.":::
...
1. On the **Create a resource group** page, fill out the values to create the resource group:
   ...
   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-multi-subnet/01-resource-group-create-complete.png" alt-text="Screenshot of the create resource group pane in the Azure portal.":::
...
1. Go to your resource group in the [Azure portal](https://portal.azure.com) and select **+ Create**
   ...
   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-multi-subnet/02-create-resource-rg.png" alt-text="Screenshot showing the create a new resource group in the Azure portal.":::
...
1. On the **Create virtual network** page, enter the following information on the **Basics** tab:
   ...
   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-multi-subnet/03-create-vnet-basics.png" alt-text="Screenshot of the basics tab of the create virtual network pane in the Azure portal.":::
...
1. On the **IP addresses** tab, select the **default** subnet to open the **Edit subnet** page. Change the name to **DC-subnet** to use for the domain controller subnet. Select **Save**.
   ...
   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-multi-subnet/04-create-vnet-ip-address-rename-default-subnet.png" alt-text="Screenshot of the Edit Subnet pane when creating a virtual network in the Azure portal.":::
...
1. Select **+ Add subnet** to add an additional subnet for your first SQL Server VM, and fill in the following values:
   ...
   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-multi-subnet/05-create-vnet-ip-address-add-sql-subnet-1.png" alt-text="Screenshot of the Add Subnet pane when creating a virtual network in the Azure portal.":::
...
1. Repeat the previous step to add an additional unique subnet range for your second SQL Server VM with a name such as **SQL-subnet-2**. You can iterate the third octet by one again.
   ...
   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-multi-subnet/06-create-vnet-ip-address-add-sql-subnet-2.png" alt-text="Screenshot of the add subnet pane in the Azure portal.":::

1. After you've added the second subnet, review your subnet names and ranges (your IP address ranges might differ from the image). If everything looks correct, select **Review + create**, and then **Create** to create your new virtual network.

   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-multi-subnet/07-create-vnet-ip-address.png" alt-text="Screenshot validating the subnets when creating a virtual network in the Azure portal.":::
...
1. Fill out the values on the **Create a virtual machine** page to create your domain controller VM, such as **DC-VM-1**. Optionally, create an additional VM, such as **DC-VM-2** to provide high availability for the Active Directory Domain Services. Use the values in the following tablet to create your VM(s):
   ...
   | **Availability options** |Availability zone <br /> *For Azure regions that don't support Availability zones, use Availability sets instead. Create a new availability set, and place all VMs created in this tutorial inside the availability set.* |
...
1. Connect to the Bastion session using your configured administrator account (**DomainAdmin**) and password (**secure password**).
1. Open the **Server Manager** dashboard (which might open by default) and choose to **Add roles and features**.

   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-multi-subnet/09-add-features.png" alt-text="Screenshot of the Server Manager - Add roles.":::

1. Select **Next** until you get to the **Server Roles** section.
1. Select the **Active Directory Domain Services** and **DNS Server** roles. When you're prompted, add any additional features that these roles require.
   ...
   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-multi-subnet/10-add-roles.png" alt-text="Screenshot of the Add Roles dialog box.":::
...
1. Select the **More** link on the yellow warning bar.
   ...
   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-multi-subnet/11-ad-ds-more.png" alt-text="Screenshot of the AD DS dialog on the DNS Server VM.":::
...
1. In the **Active Directory Domain Services Configuration Wizard**, use the following values:
   ...
   | **Domain Controller Options** |**DSRM Password** = secure password<br />**Confirm Password** = secure password |
...
:::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-multi-subnet/12-dc-vm-1-private-ip.png" alt-text="Screenshot showing the private IP address on the networking page for your VM in the Azure portal.":::
...
:::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-multi-subnet/12-identify-dns-ip-address.png" alt-text="Screenshot showing where to add a DNS server for your virtual machine in the Azure portal.":::

### Configure second domain controller

After the primary domain controller restarts, you can optionally configure the second domain controller for the purpose of high availability. If you don't want to configure a second domain controller, skip this step. However, a second domain controller is recommended in production environments.
...
1. In the **Windows Security** popup dialog, specify the credentials for the default domain administrator account (**CORP\DomainAdmin**) and the password (**secure password**).
...
1. Select the **Add roles and features** link on the dashboard.
   ...
   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-multi-subnet/09-add-features.png" alt-text="Screenshot of Server Manager - Add roles":::
...
1. Select **Select**.
1. Connect by using the administrator account (**CORP.CONTOSO.COM\domainadmin**) and password (**secure password**).
...
1. In the right-hand **Tasks** pane, verify you see **corp (local)** in the dropdown list, and then select **Properties** underneath.
   ...
   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-multi-subnet/15-ad-dc-properties.png" alt-text="Screenshot of the Active Directory Administrative Center - CORP user properties":::
...
1. Check the boxes next to **Read all properties** and **Create Computer Objects**.
   ...
   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-multi-subnet/16-add-permissions.png" alt-text="Screenshot of the Active Directory Administrative Center - Corp user permissions":::

1. Select **OK**, and then select **OK** again. Close the **corp** properties window.

Now that you've finished configuring Active Directory and the user objects, you're ready to create your SQL Server VMs.

## Create SQL Server VMs

Once your AD, DNS, and user accounts are configured, you're ready to create your SQL Server VMs. For simplicity, use the SQL Server VM images in the marketplace.
...
| **VM basics** |**Name** = SQL-VM-1<br />**User Name** = DomainAdmin<br />**Password** = secure password<br />**Subscription** = Your subscription<br />**Resource group** = SQL-HA-RG<br />**Location** = Your Azure location |**Name** = SQL-VM-2<br />**User Name** = DomainAdmin<br />**Password** = secure password<br />**Subscription** = Your subscription<br />**Resource group** = SQL-HA-RG<br />**Location** = Your Azure location |
...
> [!NOTE]  
> These suggested machine sizes are only intended for testing availability groups in Azure Virtual Machines. For optimized production workloads, see the size recommendations in [Performance best practices for SQL Server on Azure VMs](performance-guidelines-best-practices-checklist.md).
>
...
If you're on Windows Server 2019 or later, only assign a secondary IP address for the availability group listener, and skip the steps to assign a windows cluster IP. However, if you plan to configure your cluster with a virtual network name (VNN), assign both IP addresses to each SQL Server VM as you would for Windows Server 2016.
...
1. Select **Networking** in the **Settings** pane, and then select the **Network Interface**:
   ...
   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-multi-subnet/19-sql-vm-network-interface.png" alt-text="Screenshot of the virtual machine networking page in the Azure portal with the network interface field selected.":::
...
1. On the **Network Interface** page, select **IP configurations** in the **Settings** pane and then choose **+ Add** to add an additional IP address:
   ...
   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-multi-subnet/20-ip-configurations-add.png" alt-text="Screenshot of the IP configurations page for the VM network interface in the Azure portal.":::
...
1. On the **Add IP configuration** page, do the following:
   ...
   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-multi-subnet/21-add-ip-windows-cluster.png" alt-text="Screenshot of the Add IP configuration pane for the VM network interface in the Azure portal.":::
...
1. Select **+ Add** again to configure an additional IP address for the availability group listener (with a name such as **availability-group-listener**), again specifying an unused IP address in **SQL-subnet-1** such as `10.38.1.11`:
   ...
   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-multi-subnet/22-add-ip-ag-listener.png" alt-text="Screenshot of the Add IP configuration pane for the VM network interface in the Azure portal, adding an IP address for the listener.":::
...
In later versions of SQL Server, the `NT AUTHORITY\SYSTEM` account doesn't have permissions to SQL Server by default and must be granted manually.

To add the `NT AUTHORITY\SYSTEM` account and grant appropriate permissions, follow these steps:
...
1. Select **Storage account** and select **Create**, configuring it with the following values:
   ...
   1. For **Performance**, select **Standard: Recommended for most scenarios (general-purpose v2 account)**. Azure Premium Storage isn't supported for a cloud witness.
...
1. For the port, specify **TCP** and type the appropriate port numbers. See the following example:
   ...
   :::image type="content" source="./media/availability-group-manually-configure-prerequisites-tutorial-multi-subnet/17-firewall-tcp-ports.png" alt-text="Screenshot adding a new rule to the Windows Firewall.":::
...
- [Tutorial: Configure an availability group in multiple subnets (SQL Server on Azure VMs)](availability-group-manually-configure-tutorial-multi-subnet.md)
