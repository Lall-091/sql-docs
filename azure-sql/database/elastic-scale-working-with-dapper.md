...
**Sample Code**: [Elastic database tools for Azure SQL Database - Dapper integration](https://github.com/Azure/elastic-db-tools/tree/master/Samples/Dapper).
...
The **using** block with the DDR connection scopes all database operations within the block to the one shard where tenantId1 is kept. The query only returns blogs stored on the current shard, but not the ones stored on any other shards. 
...
<a id="handling-transient-faults"></a>
