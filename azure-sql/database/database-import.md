...
- To migrate a database into an [Azure SQL Managed Instance](../managed-instance/sql-managed-instance-paas-overview.md) from a bacpac file, use SQL Server Management Studio or the `SqlPackage` command-line utility. The Azure portal and Azure PowerShell are not currently supported.
...
- Importing to a database in elastic pool isn't supported through the Azure Import and Export Portal. Instead, use [SQLPackage Import](/sql/tools/sqlpackage/sqlpackage-import), or import data into a single database and then move the database to an elastic pool.
